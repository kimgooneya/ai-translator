<script lang="ts">
	...
	import { AUTO_LANGUAGE, LANGUAGES, findLanguage } from '$lib/constants/languages';
	...
	const modelDisabled = $derived(!hasActiveProvider || availableModels.length === 0);
...
>
	<div class="flex flex-col gap-1">
		...
		<Select.Root type="single" bind:value={sourceLang}>
			<Select.Trigger id="source-lang" data-testid="source-lang-select" class="w-44">
				{findLanguage(sourceLang)?.name ?? sourceLang}
			</Select.Trigger>
			<Select.Content>
				{#each allLanguages as lang (lang.code)}
					<Select.Item value={lang.code} label={lang.name}>{lang.name}</Select.Item>
				{/each}
			</Select.Content>
		</Select.Root>
	</div>
	...
	<div class="flex flex-col gap-1">
		...
		<Select.Root type="single" bind:value={targetLang}>
			<Select.Trigger id="target-lang" data-testid="target-lang-select" class="w-44">
	...
	<div class="flex flex-col gap-1">
		...
		<Select.Root type="single" bind:value={model}>
			<Select.Trigger id="model-select" data-testid="model-select" class="w-48" disabled={modelDisabled}>
				{modelTriggerLabel}
			</Select.Trigger>
			{#if !modelDisabled}
				<Select.Content>
